...
def parse_args():
    ...
    p.add_argument("--run",        default=None,
                   help="Run folder name. Defaults to LATEST_RUN.txt.")
    return p.parse_args()
...
def main():
    ...
    logger.info(f"Matching entities from dictionary and merging with BERT tags...")
    
    with open(output_path, "w", encoding="utf-8") as out_f:
        for idx, (record, tokens, bert_tags) in enumerate(zip(records, all_tokens, all_bert_tags)):
            sentence_conflicts = defaultdict(list)
            
            dict_matches = entity_dict.match(tokens)
            dict_tags = matches_to_bio(tokens, dict_matches)

            merged_tags, sources = merge(dict_tags=dict_tags, bert_tags=bert_tags, conflict_counts=sentence_conflicts)
            
            spans = iob2_to_spans(merged_tags)
            entity_types = sorted(set(s for _,_,s in spans))
            entity_count = len(spans)
            
            stats.update(merged_tags, sources, entity_count, entity_types, sentence_conflicts)
