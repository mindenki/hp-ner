...
from src.selection.selection import  dominant_label, has_conflict, is_high_confidence, distinct_entity_types, split_unique_per_annotator
...
def select_buckets(records: list[dict], targets: dict[str, int], overlap_count: int, seed: int) -> dict[str,list[dict]]:
    """ 
    ...
    if len(bucket_d) < targets["D_multi_type"]:
        logger.warning(f"Bucket D underfilled: needed {targets['D_multi_type']} but only found {len(bucket_d)}")
    ...
    if len(bucket_c) < targets["C_conflict"]:
        logger.warning(f"Bucket C underfilled: needed {targets['C_conflict']} but only found {len(bucket_c)}")
    ...
    if len(bucket_b) < targets["B_rare_types"]:
        logger.warning(f"Bucket B underfilled: needed {targets['B_rare_types']} but only found {len(bucket_b)}")
    ...
    logger.info(f"Bucket A (high-confidence): pool={len(a_pool)} selected={len(bucket_a)}")
    
    if len(bucket_a) < targets["A_high_confidence"]:
        logger.warning(f"Bucket A underfilled: needed {targets['A_high_confidence']} but only found {len(bucket_a)}")
...
def log_stats(gold: dict[str, list[dict]]) -> None:
    ...
    logger.info(f"Total overlap sentences (in multiple buckets): {len(gold['overlap'])}")
    logger.info(f"Total unique sentences in gold pool (excluding overlap): {len(all_gold_ids)}")
    ...
    logger.info(f"Total sentences per annotator: {unique_per_annotator + overlap_count}")
...
def main():
    ...
    log_stats(gold)
    logger.info("Gold selection completed. Files ready for import with setup_doccano_project.py")
