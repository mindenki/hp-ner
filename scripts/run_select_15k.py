...
def step1_hard_filter(records: list[dict]) -> list[dict]:
    ...
    logger.info(f"Step 1: Filtered out {dropped}({100*dropped/len(records):.2%}) records with no silver labels. Remaining: {len(filtered)}")
    return filtered
...
def step2_source_cap(records: list[dict], source_cap: int=SOURCE_CAP) -> list[dict]:
    ...
    for source, recs in grouped.items():
        ...
        capped.extend(recs[:min(source_cap, len(recs))]) # cap to SOURCE_CAP sentences per source or all sentences if less than SOURCE_CAP
    
    dropped = len(records) - len(capped)
    logger.info(f"Step 2: Capped to {source_cap} sentences per source. Filtered out {dropped}({100*dropped/len(records):.2%}) records. Remaining: {len(capped)}")
...
def step3_stratified_sampling(records: list[dict], targets: dict[str, int], seed: int) -> list[dict]:
    ...
    if unclassified:
        logger.warning(f"{unclassified} records had no dominant label and will be ignored in stratified sampling.")
    ...
    for label, target in targets.items():
        ...
        if shortfall > 0:
            ...
            logger.warning(f"Label {label} has shortfall of {shortfall} (target {target}, available {len(available)}). Will add to CHARACTER shortfall.")
        else:
            logger.info(f"Label {label}: target {target}, available {len(available)}, taking {take}.")
    ...
    if char_shortfall:
        ...
        if len(char_available) < char_shortfall:
            logger.warning(f"CHAR label has shortfall of {char_shortfall} but only {len(char_available)} available. Will take all available.")
            char_shortfall = len(char_available)
        
        selected.extend(char_available[:char_shortfall])
        logger.info(f"Added {char_shortfall} to CHARACTER from shortfall redistribution. Total CHARACTER selected: {len([r for r in selected if dominant_label(r) == 'CHAR'])}.")
    
    
    rng.shuffle(selected)
    logger.info(f"Step 3: Stratified sampling by dominant label. Final selected count: {len(selected)}. Distribution: { {label: len([r for r in selected if dominant_label(r) == label]) for label in TARGETS.keys()} }")
    if len(selected) < TOTAL_TARGET:
        logger.warning(f"Total selected {len(selected)} is less than total target {TOTAL_TARGET}. Consider adjusting targets or checking availability.")
    return selected
...
def log_stats(records: list[dict]) -> None:
    ...
    for n in sorted(entity_dist.keys()):
        count = entity_dist[n]
        logger.info(f"{n} entities: {count} records percentage: {count / len(records):.1%}")
