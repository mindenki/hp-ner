"""
Scrapes the aliases for canonical names from the Harry Potter wiki.
For each cannonical name, fetches the page's infobox.

Output: dict:   canonical name -> list[alias_string]
...
SESSION.headers.update({ # set a custom User-Agent to avoid being blocked by the server, and accept gzip encoding for efficiency
...
def _get_with_retry(params: dict, delay: float) -> dict | None: # same as in category_crawler.py
...
def _looks_like_person(name):
    ...
    return capitals >= 2
def _generate_person_short_forms(name):
