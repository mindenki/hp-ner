...
def _get_with_retry(params: dict, delay: float) -> dict | None:
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            if response.status_code == 429:
                ...
                print(f"    [429] rate-limited, waiting {wait:.0f}s (retry {attempt + 1}/{MAX_RETRIES})...")
                time.sleep(wait)
                continue
            ...
            if response.status_code >= 500:
                wait = BACKOFF_BASE ** attempt
                print(f"    [5xx] server error, waiting {wait:.0f}s (retry {attempt + 1}/{MAX_RETRIES})...")
                time.sleep(wait) # 😴
                continue
            return None
    ...
    print(f"    [FAILED] gave up after {MAX_RETRIES} retries")
    return None
