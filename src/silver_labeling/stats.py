...
class SilverStats:
    ...
    def update(self, merged_tags: list[str], sources: list[str], 
               entity_count: int, entity_types: list[str], conflict_counts_batch: dict[str,int]) -> None:
        ...
        if len(entity_types) >= 2:
            ...
            for i, l1 in enumerate(entity_types):
                for l2 in entity_types[i + 1:]:
                    self.label_cooccurrence[frozenset({l1, l2})] += 1 # frozenset to ensure order doesn't matter, so (CHAR, SPELL) and (SPELL, CHAR) are counted together
    ...
    def log_summary(self) -> None:
        ...
        for n in range(0, min(max_n+1, 11)):
            ...
            logger.info(f"{n:2d} entities: {count:5d} sentences {bar} percentage: {count/total:.1%}")
        
        
        above_10 = sum(count for n, count in self.entity_count_dist.items() if n > 10)
        if above_10:
            logger.info(f">10 entities: {above_10} sentences percentage: {above_10/total:.1%}")
        ...
        logger.info("\nPer-label token coverage:")
        total_tagged = 0
        for label in sorted(self.label_coutns):
            d = self.label_counts[label].get("dict", 0)
            b = self.label_counts[label].get("bert", 0)
            total_tagged = d + b
            logger.info(f"{label}: dict {d} tokens ({d/total_tagged:.1%}), bert {b} tokens ({b/total_tagged:.1%}), total {total_tagged} tokens")
        ...
        for pair, count in sorted_cooc[:20]: # Top 20 co-occurring pairs
            l1, l2 = sorted(pair)
            logger.info(f"{l1} & {l2}: {count} sentences percentage: {count/total:.1%}")
        
        
        # ── Conflict counts ─────────────────────────────────────
        logger.info("\nConflict counts (BERT label -> Dict label):")
        if self.conflict_counts:
            for conflict, count in sorted(self.conflict_counts.items(), key=lambda x: -x[1]):
                logger.info(f"{conflict}: {count} tokens percentage: {count/total:.1%}")
        else:
